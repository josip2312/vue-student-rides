<template>
    <section class="start-time route spacing">
        <h2 class="heading-2">
            Kada ćeš pokupiti putnike?
        </h2>
        <select
            name="select-time"
            id="select-time"
            ...
            v-model="selected"
        >
            <option disabled value="">Odaberi vrijeme</option>
        ...
        <button class="btn" @click="setStartTime">Dalje</button>
    </section>
</template>
...
export default {
    ...
    setup() {
        ...
        const setStartTime = () => {
            const hours = parseInt(selected.value.split(':')[0]);
            const minutes = parseInt(selected.value.split(':')[1]);
            store.state.rides.creatingRide.startTime.setHours(hours, minutes);
            router.push({ name: 'SeatsPrice' });
        };
        return {
            setStartTime,
            selected,
